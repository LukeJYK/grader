...
def grade_at(answer_path):
    ...
    ## very important!!!!!!!!!!!!!!!!!!!!!!
    cwd = os.getcwd()
    ending = "/grader/project_3" # TODO: change this line across projects
    ...
    if False: # TODO: (re)?-set this boolean if you want the entire folder or not
        assert os.system("cp -r %s %s" % (answer_path, GRADER_TEMP_FOLDER)) == 0
    else: # TODO: write specific logic for copying
        # in Project 3:
        student_files = [
            "distance_vector_node.py",
            "link_state_node.py"
        ]
        os.mkdir(GRADER_TEMP_FOLDER)
        for dirpath, dirnames, filenames in os.walk(answer_path):
            for filename in filenames:
                if filename in student_files or filename[-3:] == ".py":
                    assert os.system("cp %s %s" % (os.path.join(dirpath, filename), GRADER_TEMP_FOLDER)) == 0
    ...
    if os.system("rm -rf %s" % GRADER_TEMP_FOLDER) != 0: # it happens when testing on Moore, strange...
        ...
        kill_cmd = "ps -Af | grep '%s' | grep 'python3 sim.py' | awk '{ print $2 }' | xargs kill" % moore_username
        # assert os.system(kill_cmd) == 0
        os.system(kill_cmd) # may have gone. just re-delete!!
